...
func buildMininetPython(t Topo) string {
	return fmt.Sprintf(`# Auto-generated by Enhanced Mininet Topology Manager
...
func runRemoteMininet(config *Config, topo *Topo) error {
	...
	if err := runMininetCommand(client, config); err != nil {
		return fmt.Errorf("mininet execution failed: %w", err)
	}

	// 5) Cleanup remote file
	fmt.Printf("-> Cleaning up remote file: %s\n", config.RemotePath)
	if err := runRemoteCommand(client, fmt.Sprintf("rm -f %s", config.RemotePath)); err != nil {
...
func runMininetCommand(client *ssh.Client, config *Config) error {
